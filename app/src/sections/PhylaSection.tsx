      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-7 shadow-[0_24px_60px_rgba(0,0,0,0.35)]">
          <div className="mb-6 flex items-center justify-between">
            ...
            <span className="text-sm text-slate-500">Top 16 by total count</span>
          </div>
          <div className="space-y-2.5">
            {overall.map((p, i) => (
              <div key={p.name} className="group flex items-center gap-3">
                <div
                  className="w-44 shrink-0 truncate text-right text-[13px] italic text-slate-400"
                  title={p.name}
                >
                  {p.name}
                </div>
            ...
            <div className="flex gap-2">
              {GROUP_TABS.map((t) => (
                <button
                  ...
                  className={`rounded-full border px-4 py-1.5 text-xs font-medium tracking-wider transition-all ${
                    groupKey === t.key
                      ? 'border-[#C8A45D]/70 bg-[#C8A45D]/10 text-[#E3C888]'
          ...
          <div className="space-y-3">
            {groups.map((g) => (
              <div key={g.name} className="flex items-center gap-3">
                <div
                  className="w-28 shrink-0 truncate text-right text-[13px] text-slate-400"
                  title={`${g.name} · ${g.projects} projects`}
                >
                  {g.name}
                </div>
                <div className="flex h-6 flex-1 overflow-hidden rounded-md bg-white/[0.05]">
                  {legend.map((ph, li) => {
